...
import { PackageSearch, Building2, ShieldAlert } from 'lucide-react'
...
export const InventarioCliente = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      setLoading(true)
      setDbError(null)
      const [invRes, logRes, pedRes] = await Promise.all([
        supabase.from('inventario').select('*, clientes(nombre), empresas_logisticas(nombre)').eq('cliente_id', perfil.cliente_id).order('producto'),
        supabase.from('empresas_logisticas').select('id, nombre').eq('activo', true),
        supabase.from('pedidos').select('cliente_id, productos').eq('cliente_id', perfil.cliente_id)
      ])
  ...
  // Descomponer nombre para separar base y talla
  ...
  const filteredProducts = useMemo(() => {
    return groupedProducts.filter(prod => {
      ...
      return matchesSearch && matchesLogistica && matchesCategory;
    });
  }, [groupedProducts, searchTerm, logisticaFilter, categoryFilter]);
  ...
  if (!perfil?.cliente_id) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] text-center p-8 bg-white rounded-xl border max-w-2xl mx-auto shadow-lg space-y-4 my-8">
        <ShieldAlert className="h-16 w-16 text-orange-500 animate-pulse" />
        <h3 className="text-xl font-black text-gray-800">⚠️ Cuenta de Cliente Sin Vincular</h3>
        <p className="text-sm text-gray-500 max-w-md">
          Tu usuario no está vinculado a ninguna marca en la base de datos de producción de Supabase.
        </p>
      </div>
    )
  }
  ...
  if (dbError) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[65vh] text-center p-8 bg-white rounded-xl border max-w-3xl mx-auto shadow-lg space-y-4 my-8">
        ...
        <p className="text-xs text-gray-500">{dbError}</p>
      </div>
    )
  }
  ...
  return (
    <div className="p-1">
      ...
      <div className="flex flex-col md:flex-row md:justify-between md:items-center gap-4 mb-6">
        <div>
          <h2 className="text-2xl font-bold text-gray-800 tracking-tight">Mi Inventario</h2>
          <p className="text-xs text-gray-500">Consulta en tiempo real la disponibilidad de stock de tu marca en cada bodega</p>
        </div>
      ...
      <div className="flex flex-wrap gap-3 items-center bg-white p-3 rounded-2xl shadow-sm border border-gray-100/50 mb-5">
        ...
        <Select value={logisticaFilter} onValueChange={setLogisticaFilter}>
          <SelectTrigger className="w-[240px] h-9 text-xs rounded-xl border-gray-200"><SelectValue placeholder="Todas las Bodegas" /></SelectTrigger>
          <SelectContent>
                ...
                <div className="p-4 flex-1 flex flex-col">
                  ...
                  <div className="bg-gray-50/50 p-2.5 rounded-xl text-[10px] text-gray-500 space-y-1 mb-4">
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className="font-bold text-gray-800">{prod.totalVendidas || 0} pzas</span>
                    </div>
